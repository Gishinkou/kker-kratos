...
func init() {
	InitializeDB()
	err := _gen_test_db.AutoMigrate(&models.Account{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&models.Account{}) fail: %s", err)
	}
}
...
func Test_accountQuery(t *testing.T) {
	...
	err = _do.Create(&models.Account{})
	if err != nil {
		t.Error("create item in table <account> fail:", err)
	}

	err = _do.Save(&models.Account{})
	if err != nil {
		t.Error("create item in table <account> fail:", err)
	}

	err = _do.CreateInBatches([]*models.Account{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <account> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*models.Account{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&models.Account{}, 0, 1)
	if err != nil {
