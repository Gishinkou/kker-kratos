...
const OperationUserServiceHelloCore = "/coreService.api.v1.UserService/HelloCore"
...
func RegisterUserServiceHTTPServer(s *http.Server, srv UserServiceHTTPServer) {
	...
	r.GET("/v1/user/hello", _UserService_HelloCore0_HTTP_Handler(srv))
...
func _UserService_HelloCore0_HTTP_Handler(srv UserServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in HelloCoreRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationUserServiceHelloCore)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.HelloCore(ctx, req.(*HelloCoreRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*HelloCoreResponse)
		return ctx.Result(200, reply)
	}
}
...
type UserServiceHTTPClient interface {
	...
	HelloCore(ctx context.Context, req *HelloCoreRequest, opts ...http.CallOption) (rsp *HelloCoreResponse, err error)
...
func (c *UserServiceHTTPClientImpl) HelloCore(ctx context.Context, in *HelloCoreRequest, opts ...http.CallOption) (*HelloCoreResponse, error) {
	var out HelloCoreResponse
	pattern := "/v1/user/hello"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationUserServiceHelloCore))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
